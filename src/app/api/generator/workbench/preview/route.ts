...
import { startWorkbenchDatabasePreview } from '@/lib/server/generator/supabasePilotPreview';
...
export async function POST(req:Request) {
  try {
    ...
    await enforceRateLimit(access,'generator:workbench:preview',8,60);
    ...
    if(!run||!run.accepted||run.accepted.sourceHash!==body.sourceHash||run.contract.runtime.id!=='react-workbench'||Boolean((run.details as {deletedAt?:string}|null)?.deletedAt))return NextResponse.json({error:'No matching approved database delivery.'},{status:409,headers});
    ...
    return NextResponse.json(await startWorkbenchDatabasePreview(saved.snapshot,run.contract,delivery),{headers});
  } catch(error) {
