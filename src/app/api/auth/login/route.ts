...
export async function POST(req:Request){
  ...
  const auth=publicSupabaseConfig();if(!auth)return NextResponse.json({error:'Sign-in is temporarily unavailable. Please try again later.'},{status:503,headers:{'Cache-Control':'no-store'}});
  try{
  const store=await cookies();
  const client=createServerClient(auth.url,auth.key,{cookies:{getAll:()=>store.getAll(),setAll:values=>values.forEach(({name,value,options})=>store.set(name,value,options))}});
  ...
  const {data:profile,error:profileError}=await client.from('profiles').select('access_enabled').eq('id',data.user.id).maybeSingle();
  if(profileError)return NextResponse.json({error:'Your account could not be checked. Please try again shortly.'},{status:503,headers:{'Cache-Control':'no-store'}});
  ...
  }catch{return NextResponse.json({error:'Sign-in is temporarily unavailable. Please try again shortly.'},{status:503,headers:{'Cache-Control':'no-store'}});}
}
