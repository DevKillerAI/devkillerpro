...
Ativação online concluída em 9 de setembro de 2026. O commit 79a16e49148ab985d2f20ce1b6f36756c0329ab9 foi enviado ao GitHub, e a Vercel reportou sucesso no deployment 4L5k7VohcRV2zJwgsGFHPKErjy33. Os nove arquivos foram instalados no servidor com backup e hashes conferidos. O worker foi reiniciado às 16:36:44 UTC e confirmou estado ativo/pronto; a API permaneceu ativa. Não havia execuções em fila ou andamento antes da parada do worker.

A importação adicionou 13 documentos e preservou os 56 anteriores: a base passou a 69 documentos e 94 chunks. Os 13 documentos novos tiveram versão, estado e hash conferidos no PostgreSQL. Consultas de teste para todas as 12 famílias recuperaram a ficha correspondente entre as referências consumidas. A recuperação continua lexical/BM25; nenhuma chamada paga de IA foi feita. Sete testes executados no servidor passaram, além dos 27 testes locais registrados acima.

A página inicial pública respondeu HTTP 200, e /create encaminhou corretamente uma visita sem sessão para o login. Esta validação comprova publicação, seleção de produto e recuperação de conhecimento; não é uma nova geração completa nem uma certificação de qualidade visual dos futuros apps. Os projetos já gerados não foram modificados.
